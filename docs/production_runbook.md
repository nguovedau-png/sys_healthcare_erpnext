...
Schema `SyncOperation` và các bảng `ExternalReference`, `WebhookReceipt`, `ConsentRecord`, `AuditEvent` cần được migrate trước khi bật endpoint ERPNext upsert. Migration của external healthcare backend nằm tại `external_apps/backend/prisma/migrations/`; chạy bằng `pnpm exec prisma migrate deploy` trong một bước riêng trước khi rollout application mới bằng migration runner của PostgreSQL. Script có `IF NOT EXISTS` ở các migration phù hợp để an toàn khi retry, nhưng vẫn phải được ghi nhận trong migration ledger. Nếu rollback application thì không nên xoá cột hoặc bảng đã tạo. Các bản ghi sync chứa metadata đã redacted, không phải bản sao payload clinical đầy đủ, vì vậy retry an toàn được thực hiện bằng cách gửi lại payload gốc với cùng `tenantId`, `sourceSystem`, `sourceId` và `idempotencyKey`.

Migration and restore drill chính thức nằm tại `external_apps/backend/scripts/database-migration-drill.sh`. Operator phải cung cấp `DATABASE_URL`, `RESTORE_DATABASE_URL` và `CONFIRM_RESTORE=YES` qua secret manager hoặc shell được kiểm soát. Script chạy migration deploy, tạo `pg_dump` custom-format, kiểm tra các bảng healthcare/integration cốt lõi, restore vào target được phê duyệt và kiểm tra lại schema. Script từ chối chạy nếu thiếu `pg_dump`, `pg_restore` hoặc `psql`; không được dùng database production làm restore target.
