...
def run():
    ...
    content = [
        {
            "type": "header",
            "data": { "text": "Select a Module", "level": 1 }
        },
        {
            "type": "shortcut",
            "data": { "shortcuts": [] }
    ...
    for i, child_ws in enumerate(child_workspaces):
        # In v15, for external/workspace links, use type="URL" and set the "url" field
        label = frappe.db.get_value('Workspace', child_ws, 'title') or child_ws
        link_url = f"/app/workspace/{child_ws.lower().replace(' ', '-')}"
        
        ws_parent.append('shortcuts', {
            'label': label,
            'url': link_url, # Use "url" field for type "URL"
            'type': 'URL', 
    ...
    frappe.db.commit()
    frappe.clear_cache()
    print(f"Centralization complete. '{parent_name}' now hosts sub-modules.")
