...
class SearchService {
    ...
    async searchDiseases(query: string, page: number = 1, limit: number = 20) {
        return apiService.get('/partners/search/diseases', { search: query, page, limit });
    }

    async searchMedicines(query: string, page: number = 1, limit: number = 20) {
        return apiService.get('/partners/search/medicines', { search: query, page, limit });
    }

    async getMedicine(id: string | number) {
        return apiService.get(`/partners/search/medicines/${id}`);
    }
}
