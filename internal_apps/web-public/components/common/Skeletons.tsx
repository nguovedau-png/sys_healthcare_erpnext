'use client';

import React from 'react';
import Skeleton from '@/components/ui/Skeleton';
import Card from '@/components/ui/Card';

export const DoctorCardSkeleton = () => (
    <Card bodyStyle={{ padding: '16px' }} className="h-full rounded-xl shadow-sm border-gray-100">
        <div className="flex items-center space-x-4 mb-4">
            <Skeleton.Avatar active size={64} shape="circle" />
            <div className="flex-1">
                <Skeleton.Input active size="small" style={{ width: 120, marginBottom: 8 }} />
                <Skeleton.Input active size="small" style={{ width: 80 }} />
            </div>
        </div>
        <Skeleton active paragraph={{ rows: 2 }} />
        <div className="flex justify-between mt-4">
            <Skeleton.Button active size="small" />
            <Skeleton.Button active size="small" />
        </div>
    </Card>
);

export const HospitalCardSkeleton = () => (
    <Card bodyStyle={{ padding: '0' }} className="h-full rounded-xl shadow-sm border-gray-100 overflow-hidden">
        <Skeleton.Image style={{ width: '100%', height: 160 }} active />
        <div className="p-4">
            <Skeleton active paragraph={{ rows: 3 }} />
            <div className="flex justify-end mt-2">
                <Skeleton.Button active size="default" />
            </div>
        </div>
    </Card>
);

export const DiseaseCardSkeleton = () => (
    <Card bodyStyle={{ padding: '16px' }} className="h-full rounded-xl shadow-sm border-gray-100">
        <div className="flex gap-4">
            <Skeleton.Image style={{ width: 120, height: 80 }} active />
            <div className="flex-1">
                <Skeleton.Input active size="small" style={{ width: 80, marginBottom: 8 }} />
                <Skeleton.Input active size="small" style={{ width: '100%', marginBottom: 8 }} />
                <Skeleton.Input active size="small" style={{ width: '60%' }} />
            </div>
        </div>
    </Card>
);

export const MedicineCardSkeleton = () => (
    <Card bodyStyle={{ padding: '16px' }} className="h-full rounded-xl shadow-sm border-gray-100">
        <div className="flex gap-4">
            <Skeleton.Image style={{ width: 80, height: 80 }} active />
            <div className="flex-1">
                <Skeleton.Input active size="small" style={{ width: '80%', marginBottom: 8 }} />
                <Skeleton.Input active size="small" style={{ width: '60%', marginBottom: 8 }} />
                <Skeleton.Input active size="small" style={{ width: '40%' }} />
            </div>
        </div>
    </Card>
);

export const SkeletonList = ({ count = 4, type = 'doctor' }: { count?: number, type?: 'doctor' | 'hospital' | 'disease' | 'medicine' }) => {
    const renderSkeleton = () => {
        switch (type) {
            case 'disease':
                return <DiseaseCardSkeleton />;
            case 'medicine':
                return <MedicineCardSkeleton />;
            case 'hospital':
                return <HospitalCardSkeleton />;
            default:
                return <DoctorCardSkeleton />;
        }
    };

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: count }).map((_, i) => (
                <div key={i}>{renderSkeleton()}</div>
            ))}
        </div>
    );
};
