      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {_.map(safeData, (item, index) => (
          <div className="group relative overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all" key={index}>
            <button type="button" onClick={() => { setPhotoIndex(index); setIsOpen(true); }} className="block aspect-square relative bg-gray-100 w-full cursor-zoom-in focus:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Mở ảnh ${index + 1}`}>
              ...
              <span className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100" aria-hidden="true"><i className="fi flaticon-zoom text-white text-2xl transform scale-50 group-hover:scale-100 transition-transform duration-300" /></span>
            </button>
          </div>
        ))}
      </div>
