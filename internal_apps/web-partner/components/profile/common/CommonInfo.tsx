...
import { HiCheckBadge, HiOutlineExclamationCircle } from 'react-icons/hi2';
...
const CommonInfo: React.FC<CommonInfoProps> = ({ data, likeCount }) => {
    const { avatar, degree, name, speciality, userType, isVerified } = data;
    return (
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 pt-4 relative">
            ...
            <div className="flex-1 text-center md:text-left space-y-2">
                <div className="flex flex-col md:flex-row md:items-center gap-2 mb-1">
                    <div className="flex items-center gap-2">
                        ...
                        {isVerified ? (
                            <Tooltip title="Đã xác minh chính chủ">
                                <HiCheckBadge className="text-blue-500 text-2xl" />
                            </Tooltip>
                        ) : (
                            <Tooltip title="Tài khoản chưa xác minh">
                                <HiOutlineExclamationCircle className="text-gray-400 text-2xl" />
                            </Tooltip>
                        )}
                    </div>
                    {userType && (
