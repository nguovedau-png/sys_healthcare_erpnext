...
const Gallery: React.FC<GalleryProps> = ({ data }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 md:p-8">
      ...
      {isOpen && safeData.length > 0 && (
        <Lightbox
          mainSrc={safeData[photoIndex].img}
          nextSrc={safeData[(photoIndex + 1) % safeData.length].img}
          prevSrc={safeData[(photoIndex + safeData.length - 1) % safeData.length].img}
          onCloseRequest={() => setLbState({ ...lboxState, isOpen: false })}
          onMovePrevRequest={() =>
            setLbState({
              ...lboxState,
              photoIndex: (photoIndex + safeData.length - 1) % safeData.length
            })
          }
          onMoveNextRequest={() =>
            setLbState({
              ...lboxState,
              photoIndex: (photoIndex + 1) % safeData.length,
            })
          }
        />
      )}
