...
const Service: React.FC<ServiceProps> = ({ data }) => {
  const serviceData = React.useMemo<SpecialityService[]>(() => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 md:p-8">
        <div className="text-center py-8">
          <i className="fi flaticon-folder text-gray-300 text-5xl mb-4" />
                ...
                {_.map(item.services || [], (service, serviceIndex) => (
                  <div key={`${service.name}-${serviceIndex}`} className="flex justify-between items-center p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <span className="text-primary font-bold">
                      {typeof service.price === 'number' ? service.price.toLocaleString('vi-VN') : service.price} <sup className="text-xs text-gray-400">VNĐ</sup>
                    </span>
                  </div>
                ))}
              </div>
