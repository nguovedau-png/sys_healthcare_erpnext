...
const Intro: React.FC<IntroProps> = ({ data }) => {
  ...
  const safePhone = Array.isArray(phone) ? phone : (typeof phone === 'string' ? [phone] : []);
  const safeEmail = Array.isArray(email) ? email : (typeof email === 'string' ? [email] : []);
  const safeSocial = Array.isArray(socialContact) ? socialContact : [];
  const safeIntro = intro || { exp: '', degree: '', associationAward: '' };
