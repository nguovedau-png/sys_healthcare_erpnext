...
class MarketingService {
    ...
    // --- Charity Campaigns ---
    async getCharityCampaigns(): Promise<CharityCampaign[]> {
        const response = await apiService.get(`${this.baseUrl}/charity-campaigns`);
        return (response as any) || [];
    }

    async getCharityCampaign(id: number): Promise<CharityCampaign> {
        return apiService.get(`${this.baseUrl}/charity-campaigns/${id}`);
    }

    async donateToCharityCampaign(id: number, amount: number): Promise<CharityCampaign> {
        const response = await apiService.post(`${this.baseUrl}/charity-campaigns/${id}/donate`, { amount });
        return (response as any);
    }

    // --- Insurance Partners ---
    async getInsurancePartners(): Promise<InsurancePartner[]> {
        const response = await apiService.get(`${this.baseUrl}/insurance-partners`);
        return (response as any) || [];
    }
}
