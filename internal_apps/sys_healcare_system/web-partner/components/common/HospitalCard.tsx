    ...
    <div className="bg-white rounded-xl overflow-hidden hover:shadow-md transition-all duration-300 h-full border border-gray-100 group">
      <div className="flex p-4 gap-4 h-full">
        ...
        <div className="w-24 flex-shrink-0">
          <Link href={`/profile/${userType}/${userId}`} className="block relative aspect-square rounded-full overflow-hidden border-2 border-gray-100 group-hover:border-primary/50 transition-colors">
          ...
          <div>
            ...
            {!simpleMode && (
              <>
                ...
                {traffic.visit > 0 && (
                  <div className="flex items-center text-xs text-green-600 bg-green-50 w-fit px-2 py-0.5 rounded-full mb-2">
                    <i className="fi flaticon-eye mr-1"></i>
                    {traffic.visit} lượt khám/ngày
                  </div>
                )}
              </>
            )}
