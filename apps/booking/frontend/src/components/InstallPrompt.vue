<template>
	<Dialog v-model="showDialog">
		<template #body-title>
			<h2 class="text-lg font-bold">{{ __('Install Booking App') }}</h2>
		</template>
...
if (
	isIos() &&
	!isInStandaloneMode() &&
	localStorage.getItem('bookingIosInstallPromptShown') !== 'true'
) {
	iosInstallMessage.value = true
	localStorage.setItem('bookingIosInstallPromptShown', 'true')
}
