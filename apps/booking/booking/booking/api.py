# Copyright (c) 2026, booking and contributors
...
@frappe.whitelist()
def create_booking(doctor, booking_date, booking_time, notes=None):
    """Create a new booking"""
    try:
        # Check if user is logged in
        if not frappe.session.user or frappe.session.user == "Guest":
            return {"error": "Please login to create a booking"}
...
def get_my_bookings():
    ...
    try:
        if not frappe.session.user or frappe.session.user == "Guest":
            return {"error": "Please login to view bookings"}
        
        bookings = frappe.get_all(
            "Booking",
            fields=["name", "doctor", "doctor_name", "booking_date", "booking_time", "status", "notes"],
...
def get_translations():
    """Get translations for the booking app"""
    try:
...
def get_booking_settings():
    """Get booking app settings"""
    try:
        # Try to get from Single DocType if it exists
        if frappe.db.exists("DocType", "Booking Settings"):
            settings = frappe.get_single("Booking Settings")
            return {
