	...
	<div class="w-full px-5 pt-5 pb-10">
		...
		<div class="mb-6 flex items-center justify-between">
			...
			>
				...
				{{ __('New Gamemarketing') }}
			</Button>
		</div>

		<!-- Loading State -->
		<div v-if="gamemarketings.loading" class="flex items-center justify-center py-12">
			<div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
		</div>

		<!-- Gamemarketings List -->
		<div
			v-else-if="gamemarketings.data && gamemarketings.data.length > 0"
			class="space-y-4"
		>
			<div
				v-for="gamemarketing in gamemarketings.data"
				:key="gamemarketing.name"
					...
					<div class="flex-1">
						<div class="flex items-center space-x-3 mb-3">
							...
							<div>
								<h3 class="text-base font-semibold text-ink-gray-9">
									{{ gamemarketing.doctor_name }}
								</h3>
								<p class="text-xs text-ink-gray-5">
									{{ gamemarketing.doctor }}
								</p>
							</div>
						</div>
						...
						<div class="grid grid-cols-2 gap-4 mt-4">
							<div class="flex items-center text-sm text-ink-gray-7">
								...
								<span>{{ gamemarketing.gamemarketing_date }}</span>
							</div>
							<div class="flex items-center text-sm text-ink-gray-7">
								<Clock class="w-4 h-4 mr-2 text-ink-gray-5" />
								<span>{{ gamemarketing.gamemarketing_time }}</span>
							</div>
						</div>

						<p
							v-if="gamemarketing.notes"
							class="mt-3 text-sm text-ink-gray-6 bg-surface-menu-bar rounded p-3"
						>
							<strong>{{ __('Notes:') }}</strong> {{ gamemarketing.notes }}
						</p>
					</div>

					<!-- Status Badge -->
					<Badge
						:label="gamemarketing.status"
						:theme="getStatusTheme(gamemarketing.status)"
						variant="subtle"
					/>
				</div>
			</div>
...
const gamemarketings = createResource({
