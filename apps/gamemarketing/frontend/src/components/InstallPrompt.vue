<template>
	<Dialog v-model="showDialog">
		<template #body-title>
			<h2 class="text-lg font-bold">{{ __('Install Gamemarketing App') }}</h2>
		</template>
...
if (
	isIos() &&
	!isInStandaloneMode() &&
	localStorage.getItem('gamemarketingIosInstallPromptShown') !== 'true'
) {
	iosInstallMessage.value = true
	localStorage.setItem('gamemarketingIosInstallPromptShown', 'true')
}
